/* eslint-disable */
// Strip services — Option C, with pop-out case-study modals on each tile.

const STRIP_CASES = window.SPY_CASES || [];

function StripServices() {
  const [active, setActive] = React.useState(0);
  const [playing, setPlaying] = React.useState(true);
  const [modalId, setModalId] = React.useState(null);
  const list = STRIP_CASES.length ? STRIP_CASES : [];
  ...
  return (
    <section className="svc-section strip-section" id="services">
      <div className="wrap">
        ...
        {/* big demo */}
        <div className="strip-stage" style={{ "--case-accent": c.accent }}>
        ...
        <div className={"strip-tiles " + (playing ? "" : "paused")} role="tablist">
          {list.map((cs, i) => (
            ...
            >
              ...
              {active === i && <span className="strip-tile-progress" key={active + "-" + playing} />}
            </button>
          ))}
        </div>
      ...
      {modalCase && window.BentoModal && (
        <window.BentoModal c={modalCase} onClose={() => setModalId(null)} />
      )}
    </section>
  );
}

Object.assign(window, { StripServices });

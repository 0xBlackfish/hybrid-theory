...
## Home page composition (important)
`index.html` loads many section JSX files, but the **active** home (`<HomeApp>` in `home-v2.jsx`) renders only these:
- **Hero** (in `home-v2.jsx`)
- **`<StripServices>`** (`option-c-services.jsx`) — §01, the auto-advancing case strip. Depends on `services-d.jsx` (`WideVisualD`), `bento-services.jsx` (`BentoModal`), and `scrollspy-services.jsx` (`SPY_CASES`).
- **`<ApproachDocument>`** (`approach-document.jsx`) — §02.
- **`<TeamSection>`** (`team-section.jsx`) — §03.
- **`<InsightsFeatured>`** (`insights-featured.jsx`) — §04.
- **`<ToolsStrip>`** + **`<CTA>`** + **`<Footer>`** (in `home-v2.jsx`).

The other loaded files — `bento-services.jsx`, `scrollspy-services.jsx`, `option-b-services.jsx`, `approach-stream.jsx`, `approach-overlap.jsx`, and the older `ServicesSection`/`ApproachSection` inside `home-v2.jsx` — are **earlier explorations kept as dependencies or dead code**. Build only the active composition above; ignore the rest.

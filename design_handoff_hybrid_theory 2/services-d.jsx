/* eslint-disable */
// Hybrid Theory — Services Section, Variant D (full-width demo stage)
// Five concrete cases instead of four service categories.
...
const CASES_D = [
  ...
  },
  {
    ...
    title: "Score tutor sessions from their transcripts",
  ...
  },
  {
    id: "followups",
    kicker: "Lead nurture",
    title: "Send follow-up emails that reference what they browsed",
...
const MonoD = ({ children, size = 10, color }) => (
  <div className="mono" style={{ fontSize: size, letterSpacing: "0.1em", textTransform: "uppercase", color: color || "var(--ink-mute)" }}>{children}</div>
);

/* ---------- per-case wide visuals ---------- */

function WideVisualD({ id }) {
  if (id === "support") {
    return (
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr 280px", gap: 18, alignItems: "start" }}>
        <div style={{ background: "#fff", borderRadius: 10, padding: 16, border: "1px solid var(--rule)" }}>
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
            {[["Filter resets", 14, true], ["Bulk edit", 9, false], ["Archive vs delete", 7, false], ["Quickbooks sync", 5, false], ["Mobile crash", 4, false], ["Slow export", 3, false]].map(([t, n, active], i) => (
              <div key={i} style={{
                display: "flex", justifyContent: "space-between", padding: "8px 10px",
                background: active ? "var(--paper-2)" : "transparent", borderRadius: 6, fontSize: 12,
                color: active ? "var(--ink)" : "var(--ink-soft)", fontWeight: active ? 500 : 400,
              }}>
                <span>{t}</span>
...
function ServicesSectionD() {
  const [active, setActive] = React.useState(0);
  const c = CASES_D[active];
  return (
    <section className="svc-section svc-d" id="services">
      <div className="wrap">
        <header className="sec-overhead">
          ...
          <div className="sec-sub">
            <p>Pick the activity that sounds familiar. The demo is real — that's the shape of the build.</p>
          </div>
        </header>

        {/* full-width demo stage — leads */}
        <div className="svcd-stage">
          <div className="svcd-stage-bar">
            <div className="mono svcd-stage-l">Live demo · {c.kicker}</div>
            <div className="svcd-stage-r">
              <span className="svcd-dot" />
        ...
        {/* tabs sit UNDER the visual, Figma-style */}
        <div className="svcd-tabs">
          {CASES_D.map((cs, i) => (
            <button
              key={cs.id}
              onClick={() => setActive(i)}
              className={"svcd-tab " + (active === i ? "active" : "")}
            >
              <span className="svcd-tab-n mono">0{i + 1}</span>
              <span className="svcd-tab-l">{cs.kicker}</span>
            </button>
          ))}
        </div>

        {/* compact title + description + CTAs underneath */}
        <div className="svcd-head">
          <div>
            <h3 className="svcd-h">{c.title}</h3>
            <p className="svcd-desc">{c.desc}</p>
          </div>
          <div className="svcd-cta">
            <a className="btn btn-primary" href="#contact">Talk about a build <span style={{ opacity:0.6, fontSize:11 }}>↗</span></a>
...
Object.assign(window, { ServicesSectionD, WideVisualD });

...
const baseStyle = { display: "inline-block", verticalAlign: "middle" } as const;

export function LogoWordmark({ size = 22, color = "currentColor" }: { size?: number; color?: string }) {
  const w = size * (147 / 21);
  return (
    <svg width={w} height={size} viewBox="2 96 145 22" fill={color} style={baseStyle}>
      {WORDMARK_PATHS}
    </svg>
  );
}
...
export function LogoMark({ size = 28, color = "currentColor" }: { size?: number; color?: string }) {
  const w = size * (127 / 90);
  return (
    <svg width={w} height={size} viewBox="9 1 127 91" fill={color} style={baseStyle}>
      {MARK_PATHS}
    </svg>
  );
}

export function LogoLockup({ size = 22, color = "currentColor" }: { size?: number; color?: string }) {
  const w = size * (150 / 118);
  return (
    <svg width={w} height={size} viewBox="0 0 150 118" fill={color} style={baseStyle}>
      {MARK_PATHS}
      {WORDMARK_PATHS}
    </svg>
  );
...
export function Logo({ variant = "wordmark", size = 22, color }: LogoProps) {

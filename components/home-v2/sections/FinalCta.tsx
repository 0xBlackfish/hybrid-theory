export function FinalCta() {
  return (
    <section id="book" className="htv-final">
      <div className="htv-wrap">
        <span className="htv-kicker" style={{ justifyContent: "center", display: "inline-flex" }}>
          Your move
        </span>
        <h2 style={{ marginTop: "20px" }}>See the business you could be running.</h2>
        <p>
          Fifteen minutes, no slides, no commitment &mdash; just a clear read on the one change that
          would pay for itself the fastest.
        </p>
        <a href="#" data-calendly className="btn btn-primary" style={{ fontSize: 16, padding: "15px 28px" }}>
          Book your free 15-min AI Assessment
        </a>
      </div>
    </section>
  );
}
